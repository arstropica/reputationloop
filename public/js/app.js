...
var app = angular.module('RLService', [ 'ngRoute', 'ngResource', 'ngSanitize',
		'ngAnimate', 'ui.bootstrap', 'ui.bootstrap-slider' ]);
...
app
		.controller(
				...
				[
						...
						'$sce',
						'Business',
						function($scope, $routeParams, $sce, Business) {
												...
												if (typeof data == 'object') {
													if (data.data !== undefined) {
														if (data.data.business_address !== undefined) {
															...
															$scope.businessInfo.data.business_address = $sce
																	.trustAsHtml($scope.businessInfo.data.business_address);
 ...
 */
app.controller("BusinessReviewsCtrl", [
		'$scope',
		'$routeParams',
		'$sce',
		'Reviews',
		function($scope, $routeParams, $sce, Reviews) {
			...
			$scope.get = function() {
				...
				Reviews.get(params).$promise.then(function(data) {
					...
					if (data.count !== undefined) {
						...
						$scope.pager.total = Math.ceil(data.count
								/ $scope.pager.max);
					}
				});
			};
						...
						link : function(scope /* , element, attrs */) {
							...
							scope.$watch('expanded', function(expandedNew) {
								if (expandedNew) {
									...
									scope.text = $filter('truncate')(
											originalText, scope.limit, '...');
									scope.label = scope.labelExpand;
								}
							});
